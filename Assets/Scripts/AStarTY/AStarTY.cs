...
namespace Exploration {
	...
	public class AStarTY : System.Collections.IComparer {
		...
		private List<Node> getAdjacent (Node n, Node[][][] matrix) {
			...
			if( n.t + 1 > matrix.Length - 1 )
				return adj;
			for (int xx = n.x - 1; xx <= n.x + 1; xx++) {
				for (int yy = n.y - 1; yy <= n.y +1; yy++) {
					if ( xx >= 0 && xx < matrix[0].Length && yy >= 0 && yy < matrix [0][0].Length) {
						Node c = matrix [n.t + 1] [xx] [yy];
